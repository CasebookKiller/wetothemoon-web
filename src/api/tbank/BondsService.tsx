...
export const BondsService = {
  ...
  getBondsData(bonds?: IBond[]) {
    return bonds;
  },

  getBondsMini(bonds?: IBond[]) {
    const bondsData = this.getBondsData(bonds);
    if (!bondsData) {
        throw new Error("getBondsData returned undefined");
    }
    return Promise.resolve(bondsData.slice(0, 5));
  },

  getBondsSmall(bonds?: IBond[]) {
    const bondsData = this.getBondsData(bonds);
    if (!bondsData) {
        throw new Error("getBondsData returned undefined");
    }
    return Promise.resolve(bondsData.slice(0, 10));
  },

  // вставка для облигаций
  getBonds(bonds: IBond[]) {
    const bondsData = this.getBondsData(bonds);
    if (!bondsData) {
        throw new Error("getBondsData returned undefined");
    }
    return Promise.resolve(bondsData);
  },

  // код из примера, не используется
...
/*
{
      ...
      "maturityDate": "2025-12-02T00:00:00Z",
      "nominal": {
        "currency": "rub",
        "units": "0",
        "nano": 0
      ...
      "stateRegDate": "2023-12-06T00:00:00Z",
      "placementDate": "2023-12-13T00:00:00Z",
      "placementPrice": {
        "currency": "rub",
        "units": "1000",
        "nano": 0
